...
for train_features, train_labels in test_loader:
    # print(train_features.numpy(), train_labels.numpy())
    testing_inputs.append(train_features.numpy())
    testing_labels.append(train_labels.numpy())

num_samples = 500
testFreqs = np.empty([num_samples, 2])

inputs = 2
hidden_neurons = 5
outputs = 3
weights = []
weights.append(np.loadtxt("input_weights_2.txt"))
weights.append(np.loadtxt("output_weights_2.txt"))
...
cs = np.empty(num_samples)

# toPlot = []
for trial in range(num_samples):
    print(trial)
    # for batch in range(batch_size):
    freq1 = (testing_inputs[trial][0][0] * 15)
    freq2 = (testing_inputs[trial][0][1] * 15)
    correct_output = testing_labels[trial][0]
...
fig = plt.figure()
# xs = np.array(toPlot)
plt.scatter(xs[cs == 0], ys[cs == 0], color='C0', edgecolor='k', alpha=0.7)
plt.scatter(xs[cs == 1], ys[cs == 1], color='C1', edgecolor='k', alpha=0.7)
plt.scatter(xs[cs == 2], ys[cs == 2], color='C2', edgecolor='k', alpha=0.7)
