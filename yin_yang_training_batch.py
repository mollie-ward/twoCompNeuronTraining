...
# weights = []
# weights.append(np.loadtxt("input_weights_1.txt"))
# weights.append(np.loadtxt("output_weights_1.txt"))

weights.append(np.zeros([inputs, hidden_neurons]))
weights.append(np.zeros([hidden_neurons, outputs]))
for layer in range(np.size(weights)):
    ...
    for input in range(np.size(these_weights, 0)):
        for synapse in range(np.size(these_weights, 1)):
            weights[layer][input][synapse] = np.random.randint(-5, 5)

loss_array.append(np.zeros([inputs, hidden_neurons, batch_size]))
loss_array.append(np.zeros([hidden_neurons, outputs, batch_size]))
gradient_array.append(np.zeros([inputs, hidden_neurons]))
gradient_array.append(np.zeros([hidden_neurons, outputs]))
...
trainingFreqs = np.empty([num_samples,2])
testFreqs = np.empty([int(num_samples / 4), 2])
...
for epoch in range(num_epochs):
    ...
    print("epoch = ", epoch, "loss = ", np.mean(total_loss), "learning rate = ", lr, "dw = ", dw)
    loss_array = []
    gradient_array = []
    output_array = []
    total_loss = []
        ...
        for layer in range(np.size(weights)):
            ...
            for input in range(np.size(these_weights,0)):
                for synapse in range(np.size(these_weights,1)):
                    ...
                    gradient_array[layer][input][synapse] += (this_loss - loss)/dw

                    weights[layer][input][synapse] -= dw

        if trial > 0 and trial % 9 == 0:
            batch += 1
            mean_gradient_array[0] = gradient_array[0]/batch_size
            mean_gradient_array[1] = gradient_array[1]/batch_size
            for layer in range(np.size(weights)):
                these_weights = weights[layer]
                for input in range(np.size(these_weights,0)):
                    for synapse in range(np.size(these_weights,1)):
                        weights[layer][input][synapse] -= mean_gradient_array[layer][input][synapse] * lr
            print("Epoch = ", epoch, "Trial = ", trial, "Batch = ", batch, "weights updated", "lr = ", lr, "dw = ", dw)
            np.savetxt("input_weights_"+str(run)+".txt", weights[0])
            np.savetxt("output_weights_"+str(run)+".txt", weights[1])
...
#     output_array = []
#     total_loss = []
#     loss_array.append(np.zeros([inputs, hidden_neurons, num_samples]))
#     loss_array.append(np.zeros([hidden_neurons, outputs, num_samples]))
#     gradient_array.append(np.zeros([inputs, hidden_neurons, num_samples]))
#     gradient_array.append(np.zeros([hidden_neurons, outputs, num_samples]))
#     output_array.append(np.zeros([inputs, hidden_neurons, outputs]))
#     output_array.append(np.zeros([hidden_neurons, outputs, outputs]))
#
#     for trial in range(num_samples):
...
#             for input in range(np.size(these_weights, 0)):
#                 for synapse in range(np.size(these_weights, 1)):
#                     print("Trial = ", trial, "batch = ", batch, "Layer = ", layer, "Input = ", input, "Synapse = ", synapse)
#                     weights[layer][input][synapse] += dw
#                     these_outputs = run_suimulation(freq1, freq2, weights)
#                     normalised_output = [(these_outputs[0] / max_firing_rate), (these_outputs[1] / max_firing_rate), (these_outputs[2] / max_firing_rate)]
#                     output_array[layer][input][synapse][:] = normalised_output
